...
STOCK_DATASET_PATH = FINAL_DIR / 'global_stocks.csv'


def load_final_datasets(symbols: Iterable[str] = DEFAULT_SYMBOLS) -> pd.DataFrame:
    if not STOCK_DATASET_PATH.exists():
        raise FileNotFoundError(f'Combined stocks dataset not found: {STOCK_DATASET_PATH}')

    combined = pd.read_csv(STOCK_DATASET_PATH).rename(columns={'Target': TARGET_COLUMN})
    required_columns = {DATE_COLUMN, SYMBOL_COLUMN, TARGET_COLUMN, 'Close'}
    missing = required_columns - set(combined.columns)
    if missing:
        raise ValueError(f'Combined stocks dataset is missing columns: {", ".join(sorted(missing))}')
    ...
    if missing_symbols:
        raise ValueError(f'Combined stocks dataset is missing symbols: {", ".join(sorted(missing_symbols))}')
